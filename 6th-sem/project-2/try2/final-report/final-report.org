...
To utilize this flashcard, we would:
1. read the question
2. try to remember the answer.
3. then, flip the card & see how well we remembered it.

CardsQL is a web app that digitises this technique in the following ways: 
1. Users can create cards by entering question, answer text 
2. Each card is stored in a database along with some metadata.
3. Users can then practice/review cards,
   + Only the question is shown at first and user is prompted to remember the answer.
   + After attempting to remember it, users can press a button to reveal the answer.
4. Users can rate how well they remembered it,
   + rating is sent to an algorithm to decide when the card should be practiced again.

This algorithm, along with a few scientific theories related to making such a revision technique
...
CardsQL is designed such that users only need to use it for a short time regularly, in order to revise topics effectively. When revising, the app will only show questions that the user is struggling with or may have forgotten. 
...
+ *cumbersome to schedule, keep track of next repition for cards*   
  - After reviewing a card, we need to schedule when to practice it again.
...
** Objectives
- To create an easy to use flashcard program.
- To promote regular study habit among students by making it effortless and quick.
- To improve rate of retaining studied knowledge among users

** Scope and Limitation
*** Scope
CardsQL would be appropriate for the following types of users:
- Students ranging from high schooolers to Doctorates, as they have to understand and memorize a lot of concepts. Flashcard applications like *Remnote, Anki* are popular among college students, especially those in medical fields.
- People learning a new language. Applications like *Duolingo, Anki* that use question-answer based learning are popularly
...
- Currently, all flashcards are stored in a single table in the database. This may become unorganized or have performance issues after a large number
  of cards are added.
** Report Organization
...
does and the problems it sets out to achieve. The chapter also talks about suitable users for CardsQL and some of its
...
requirements and figuring out various designs for making CardsQL work. [[*Implementation and Testing][_Chapter 4_]] similarly describes how the plans were implemented and briefly goes through the inner workings of the project.
...
# +begin_src plantuml :file diagrams/use-case-diagram.svg :exports results  
#+begin_src plantuml :file diagrams/use-case-diagram.svg :exports none  
  @startumL
...
- There were no development costs as we only used existing hardware & freely-licensed tools. 
...
#+begin_src plantuml :file diagrams/database-schema.svg  :exports results
  @startuml
  skinparam Padding 2
  entity "cards" {
...
- CSS, without any frameworks, was used for appplying styling and decorations.
**** Logic layer
...
CardsQL uses a modified version of SM-2, described below.
...
- The repetition number =N=, which is the consecutive number of times the card has been successfully recalled
  (meaning Q \ge 2).  Rating a card below 2 resets =N= to 0.
...
- The easiness factor =EF=, which loosely indicates how "easy" the card is (More precisely, it determines how quickly the
  ...
  Similarly, it should be ~<=2.5~ so that it isn't scheduled so far into the future that we've forgotten the answer completely. 
...
- For Test 2, form for submitting rating only has inputs with specified set of values(0-3). In case its value is
  ...
  #+end_src
  If rating is below 0, it's set to 0 and if it is above 3, it's set to 3.
...
- Provide a simple introduction to using flashcards & spaced repititon for learning
...
longer periods of time. Students and programmers might also take inspiration to build their own flashcard app.
Building projects like this is a great way to learn new things as well as solidify previously learned
